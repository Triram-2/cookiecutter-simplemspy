...
std_logger.setLevel(logging.INFO) # Убедимся, что уровень позволяет этому сообщению пройти

if __name__ == "__main__":
    std_logger.info("Это тестовое сообщение от стандартного logging.") # Это сообщение должно быть перехвачено
    log.info(f"Запуск Uvicorn сервера на http://{settings.app_host}:{settings.app_port}")
    log.info(f"Автоперезагрузка при изменениях кода: {'Включена' if settings.app_reload else 'Выключена'}")
    log.info(f"Для остановки сервера нажмите CTRL+C")
