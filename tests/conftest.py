...
from httpx import AsyncClient, ASGITransport # ASGITransport добавлен
...
async def async_client(override_get_async_session: None) -> AsyncGenerator[AsyncClient, None]:
    ...
    transport = ASGITransport(app=fastapi_app) # Создаем транспорт
    async with AsyncClient(transport=transport, base_url="http://testserver") as client: # Передаем транспорт
        yield client
